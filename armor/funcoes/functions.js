...
// i.id pode vir em formato @lid (modo de endereçamento atual do WhatsApp).
// i.phoneNumber é o telefone real correspondente, QUANDO o WhatsApp manda
// esse campo pronto na metadata — nem sempre vem preenchido pra todo
// participante. Quando falta, tenta resolver via cache da própria lib
// antes de cair no @lid cru (que nunca bate com o `sender`, já resolvido
// pro telefone no index.js/iniciar.js).
const idResolvido = i.phoneNumber
    || (i.id?.endsWith('@lid') ? resolveJid(i.id) : null)
    || i.id
if(i.admin == 'admin') admins.push(idResolvido)
if(i.admin == 'superadmin') admins.push(idResolvido)
}
return admins
}
...
admins = []
for (let i of participants) {
if(i.admin == null) admins.push(i.phoneNumber || i.id)
}
return admins
}
